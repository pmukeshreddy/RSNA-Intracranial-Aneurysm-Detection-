class PhysicsInformedSwinUNETR(nn.Module):
    def __init__(self, img_size=(64,128,128), in_channels=3, num_classes=14, 
                 feature_size=48, depths=(2, 2, 2, 2), num_heads=(3, 6, 12, 24), 
                 window_size=7, drop_rate=0.0, attn_drop_rate=0.0, use_3d=False, 
                 use_checkpoint=False, predict_flow=True):
        super().__init__()
        
        # Store the original SwinUNETR parameters
        self.use_3d = use_3d
        ...
        # Aneurysm classification head (original)
        ...
        # NEW: Velocity field prediction head
        if self.predict_flow and use_3d:
            self.velocity_head = nn.Sequential(
                Conv(feature_size, feature_size // 2, kernel_size=3, padding=1),
                nn.BatchNorm3d(feature_size // 2),
                nn.ReLU(inplace=True),
                Conv(feature_size // 2, feature_size // 4, kernel_size=3, padding=1),
                nn.BatchNorm3d(feature_size // 4),
                nn.ReLU(inplace=True),
                Conv(feature_size // 4, 3, kernel_size=1)  # 3D velocity field (vx, vy, vz)
            )
            ...
            self.pressure_head = nn.Sequential(
                ...
                nn.BatchNorm3d(feature_size // 2),
                nn.ReLU(inplace=True),
                Conv(feature_size // 2, 1, kernel_size=1)  # Scalar pressure field
            )
    ...
    def forward(self, x):
        ...
        aneurysm_pred = self.aneurysm_head(x)
        aneurysm_pred = aneurysm_pred.view(batch_size, self.num_classes)

        outputs = {'aneurysm': aneurysm_pred}

        # Add physics predictions if enabled
        if self.predict_flow and self.use_3d:
            velocity_field = self.velocity_head(x)  # Shape: (B, 3, D, H, W)
            pressure_field = self.pressure_head(x)  # Shape: (B, 1, D, H, W)
            
            outputs['velocity'] = velocity_field
            outputs['pressure'] = pressure_field
